...
- **Install hooks from the checkout you commit from.** In linked worktrees, rerun `make install-hooks` there so the shared git hook points at that checkout's scripts.
...
- **Commit messages reference td.** Use `td commit-message 'brief summary'`, or let the `commit-msg` hook normalize the first line to `feat|fix|docs|test|chore|ci: <summary> (td-<id>)` while preserving optional breaking-change `!` markers.
