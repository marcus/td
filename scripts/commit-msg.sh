...
is_git_generated_subject() {
  local subject="$1"
  [[ "$subject" == Merge* || "$subject" == "fixup!"* || "$subject" == "squash!"* || "$subject" == Revert* ]]
}
...
normalize_subject() {
  ...
  if [[ "$core_subject" =~ ^([[:alpha:]]+)[[:space:]]*\(([[:alnum:][:space:]_./-]+)\)[[:space:]]*[:-][[:space:]]*(.+)$ ]]; then
    type_raw="${BASH_REMATCH[1]}"
    scope_raw="${BASH_REMATCH[2]}"
    summary_raw="${BASH_REMATCH[3]}"
    type=$(normalize_type "$type_raw") || fail "unsupported commit type '$type_raw'"
  ...
  if [[ "$core_subject" =~ ^([[:alpha:]]+)[[:space:]]*[:-][[:space:]]*(.+)$ ]]; then
    type_raw="${BASH_REMATCH[1]}"
