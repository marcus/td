...
MSG_FILE="${1:-}"
ALLOWED_TYPES_REGEX='feat|fix|docs|test|chore|ci|perf|refactor|style|release'
...
normalize_subject() {
  local subject="$1"
  local cleaned parsed type scope summary

  cleaned="$(trim_and_collapse "$subject")"

  parsed="$(parse_subject "$cleaned" '^([A-Za-z]+)(\([^)]+\))?[[:space:]]*:[[:space:]]*(.+)$')"
  if [[ -n "$parsed" ]]; then
...
EOF
  ...
  parsed="$(parse_subject "$cleaned" '^([A-Za-z]+)(\([^)]+\))?[[:space:]]+(.+)$')"
  if [[ -n "$parsed" ]]; then
    IFS='|' read -r type scope summary <<EOF
$parsed
EOF
...
strip_trailing_refs() {
  local subject="$1"
  local stripped="$subject"
  local next=""

  while true; do
    next="$(printf '%s\n' "$stripped" | sed -En 's/^(.*)[[:space:]]\((td-[[:alnum:]]+|#[0-9]+)\)$/\1/p' | head -n 1)"
    if [[ -z "$next" || "$next" == "$stripped" ]]; then
      break
    fi
    stripped="$next"
  done

  trim_and_collapse "$stripped"
}
