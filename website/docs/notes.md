...
Use notes for freeform project context that does not belong to a single issue. Notes live alongside your td data, but they are separate from the issue workflow.
...
- meeting notes, research snippets, and rough plans
...
Use issues when the work has a clear owner, status, review path, or acceptance criteria.
...
If you omit `--content`, td opens your `$EDITOR` so you can write a longer note comfortably:
...
For the full remote setup, see [Sync & Collaboration](./sync-collaboration.md).
