...
func runBootstrap(database *db.DB, client *syncclient.Client, state *db.SyncState) (*db.DB, error) {
	...
	if err := os.WriteFile(dbPath, snapshot.Data, 0644); err != nil {
		if renameErr := os.Rename(backupPath, dbPath); renameErr != nil {
			return nil, fmt.Errorf("write snapshot: %w (restore backup: %v)", err, renameErr)
		}
		reopened, reopenErr := db.Open(baseDir)
		if reopenErr != nil {
	...
	if err != nil {
		if renameErr := os.Rename(backupPath, dbPath); renameErr != nil {
			return nil, fmt.Errorf("reopen after bootstrap: %w (restore backup: %v)", err, renameErr)
		}
	...
	if err != nil {
		reopened.Close()
		if renameErr := os.Rename(backupPath, dbPath); renameErr != nil {
			return nil, fmt.Errorf("update sync_state: %w (restore backup: %v)", err, renameErr)
		}
		reopened2, reopenErr := db.Open(baseDir)
		if reopenErr != nil {
