...
func TestWsStartWithActiveSessionErrors(t *testing.T) {
	...
	database.CreateWorkSession(ws) //nolint:errcheck // test setup
	config.SetActiveWorkSession(dir, ws.ID) //nolint:errcheck // test setup
...
func TestWsStopEndsSession(t *testing.T) {
	...
	// End session
	config.ClearActiveWorkSession(dir) //nolint:errcheck // test setup
...
func TestWsTagMultipleIssues(t *testing.T) {
	...
	for _, issue := range issues {
		database.CreateIssue(issue) //nolint:errcheck // test setup
		database.TagIssueToWorkSession(ws.ID, issue.ID, "test-session") //nolint:errcheck // test setup
	}
...
func TestWsUntagPartialRemoval(t *testing.T) {
	...
	database.TagIssueToWorkSession(ws.ID, issue1.ID, "test-session") //nolint:errcheck // test setup
	database.TagIssueToWorkSession(ws.ID, issue2.ID, "test-session") //nolint:errcheck // test setup

	// Untag only issue1
	database.UntagIssueFromWorkSession(ws.ID, issue1.ID, "test-session") //nolint:errcheck // test setup
