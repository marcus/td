...
func TestCommitMessageCommandAllowsArbitraryWordPrefixWithExplicitType(t *testing.T) {
	dir := t.TempDir()
	...
	got, err := runCommitMessageCommand(t, dir, []string{"HTTP: retry on EOF"}, "", "fix", "")
	if err != nil {
		t.Fatalf("commitMessageCmd.RunE returned error: %v", err)
	}

	want := "fix: HTTP: retry on EOF (" + issue.ID + ")"
	if got != want {
		t.Fatalf("output = %q, want %q", got, want)
	}
}

func TestCommitMessageCommandAllowsArbitraryWordPrefixWithFocusedIssueType(t *testing.T) {
	dir := t.TempDir()
	...
	got, err := runCommitMessageCommand(t, dir, []string{"HTTP: retry on EOF"}, "", "", "")
	if err != nil {
		t.Fatalf("commitMessageCmd.RunE returned error: %v", err)
	}
...
func TestCommitMessageCommandAllowsArbitraryWordPrefixForNoIssueDocsOverride(t *testing.T) {
	dir := t.TempDir()

	got, err := runCommitMessageCommand(t, dir, []string{"CLI: tighten help text"}, "", "docs", "")
	if err != nil {
		t.Fatalf("commitMessageCmd.RunE returned error: %v", err)
	}

	want := "docs: CLI: tighten help text"
	if got != want {
		t.Fatalf("output = %q, want %q", got, want)
	}
}

func TestCommitMessageCommandPreservesBreakingChangeMarker(t *testing.T) {
	dir := t.TempDir()
	...
	got, err := runCommitMessageCommand(t, dir, []string{" FeAtUrE ( cli ) ! : drop deprecated flags (" + strings.ToUpper(issue.ID) + ")"}, "", "", "")
	if err != nil {
		t.Fatalf("commitMessageCmd.RunE returned error: %v", err)
	}

	want := "feat(cli)!: drop deprecated flags (" + issue.ID + ")"
	if got != want {
		t.Fatalf("output = %q, want %q", got, want)
	}
}
...
func TestCommitMessageCommandReturnsClearErrorsForMalformedInput(t *testing.T) {
	...
	_, err = runCommitMessageCommand(t, t.TempDir(), []string{"build: ship release"}, "", "", "")
	if err == nil {
...
func TestInstallHooksUsesActiveWorktreeRootInLinkedWorktree(t *testing.T) {
	...
	wantPreCommit := filepath.Join(wtPath, "scripts", "pre-commit.sh")
	wantCommitMsg := filepath.Join(wtPath, "scripts", "commit-msg.sh")
	assertSamePath(t, wantPreCommit, preCommitTarget)
	assertSamePath(t, wantCommitMsg, commitMsgTarget)
	if strings.Contains(preCommitTarget, repo) || strings.Contains(commitMsgTarget, repo) {
		t.Fatalf("hook targets should point at the active worktree path %q, not the shared checkout %q", wtPath, repo)
	}
}
