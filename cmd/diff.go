...
func runDiff(cmd *cobra.Command, args []string) error {
	...
	summaries := diff.Summarize(parsed, ref)
...
func ParseDiffOutput(rawDiff, ref string) []diff.FileSummary {
	parsed := diff.Parse(rawDiff)
	return diff.Summarize(parsed, ref)
}

// SplitLines is a test helper that cleans output for assertion.
