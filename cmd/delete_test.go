...
func TestDeleteAlreadyDeletedIssue(t *testing.T) {
	...
	// Delete once
	must(t, database.DeleteIssue(issue.ID))
...
func TestDeleteWithDependencies(t *testing.T) {
	...
	// Add dependency
	must(t, database.AddDependency(child.ID, parent.ID, "depends_on"))
...
func TestDeletePreservesIssueData(t *testing.T) {
	...
	// Delete the issue
	must(t, database.DeleteIssue(issueID))
...
func TestDeleteMultipleWithMixedStatuses(t *testing.T) {
	...
	// Delete all
	for _, id := range issueIDs {
		must(t, database.DeleteIssue(id))
	}
