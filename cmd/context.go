...
var usageCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !compact && !quiet {
			...
			fmt.Println("     Prefer an independent reviewer sub-agent. Reviewer can")
			fmt.Println("     `td approve <id> --record-only --reason \"...\"` to record approval without")
			fmt.Println("     closing; any session may then close with `td approve <id> --reason \"...\"`.")
			fmt.Println("     Trusted mode (default): if you reviewed the diff yourself, close with")
			fmt.Println("     `td approve <id> --self-review --reason \"...\"` instead of spawning a session.")
			fmt.Println()
			...
			fmt.Println()
			fmt.Println("IMPORTANT (review_policy_mode=trusted, the default): PREFER delegating review")
			fmt.Println("  to an independent sub-agent. Reviewer runs")
			fmt.Println("  `td approve <id> --record-only --reason \"...\"`, then any session closes")
			fmt.Println("  with `td approve <id> --reason \"using recorded approval\"`.")
			fmt.Println("  If you reviewed your own diff and delegation is impractical, acknowledge it:")
			fmt.Println("  `td approve <id> --self-review --reason \"...\"` (audited; no fake session needed).")
			fmt.Println("  Pin review_policy_mode=delegated|strict for the hard no-self-review wall.")
			...
			fmt.Println("  Sessions track implementers for the audit trail—use a real reviewer sub-agent")
			fmt.Println("  for independent review, or `--self-review` to acknowledge a self-review.")
