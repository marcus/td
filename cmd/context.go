...
var usageCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !compact && !quiet {
			...
			fmt.Println("     Prefer an independent reviewer when practical; in trusted mode they run")
			...
			fmt.Println("     `td approve <id> --self-review --reason \"...\"`.")
			fmt.Println()
			...
			fmt.Println("  When self-review is appropriate, acknowledge it explicitly:")
			fmt.Println("  `td approve <id> --self-review --reason \"...\"` (audited).")
			fmt.Println("  A reviewer can instead attest without closing:")
			fmt.Println("  `td approve <id> --record-only --reason \"...\"`, and any session closes after.")
			fmt.Println("  Pin delegated|strict for a hard no-self-review wall.")
