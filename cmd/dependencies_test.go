...
func TestWouldCreateCycleSimple(t *testing.T) {
	...
	mustAddDependency(t, database, issue2.ID, issue1.ID, "depends_on")
...
func TestWouldCreateCycleTransitive(t *testing.T) {
	...
	mustAddDependency(t, database, issue2.ID, issue1.ID, "depends_on")
	mustAddDependency(t, database, issue3.ID, issue2.ID, "depends_on")
...
func TestGetTransitiveBlockedDirect(t *testing.T) {
	...
	mustCreateIssue(t, database, blocker)
	mustCreateIssue(t, database, blocked1)
	mustCreateIssue(t, database, blocked2)

	// Both blocked1 and blocked2 depend on blocker
	mustAddDependency(t, database, blocked1.ID, blocker.ID, "depends_on")
	mustAddDependency(t, database, blocked2.ID, blocker.ID, "depends_on")
...
func TestGetTransitiveBlockedChain(t *testing.T) {
	...
	mustCreateIssue(t, database, issue2)
	mustCreateIssue(t, database, issue3)
	mustCreateIssue(t, database, issue4)

	mustAddDependency(t, database, issue2.ID, issue1.ID, "depends_on")
	mustAddDependency(t, database, issue3.ID, issue2.ID, "depends_on")
	mustAddDependency(t, database, issue4.ID, issue3.ID, "depends_on")
...
func TestGetTransitiveBlockedDiamond(t *testing.T) {
	...
	mustCreateIssue(t, database, top)
	mustCreateIssue(t, database, mid1)
	mustCreateIssue(t, database, mid2)
	mustCreateIssue(t, database, bottom)

	mustAddDependency(t, database, mid1.ID, top.ID, "depends_on")
	mustAddDependency(t, database, mid2.ID, top.ID, "depends_on")
	mustAddDependency(t, database, bottom.ID, mid1.ID, "depends_on")
	mustAddDependency(t, database, bottom.ID, mid2.ID, "depends_on")
...
func TestBuildCriticalPathSkipsClosedIssues(t *testing.T) {
	...
	mustCreateIssue(t, database, openIssue)
	mustCreateIssue(t, database, closedIssue)
...
func TestDepAddDependsOnFlag(t *testing.T) {
	...
	// Reset
	mustSetFlag(t, depAddCmd.Flags(), "depends-on", "")
}
...
func TestAddDependencyMultiple(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mustCreateIssue(t, database, mainIssue)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupChain: func(database *db.DB, issues []*models.Issue) {
				// issue1 -> issue2
				mustAddDependency(t, database, issues[1].ID, issues[0].ID, "depends_on")
			},
		...
		},
		{
			...
			setupChain: func(database *db.DB, issues []*models.Issue) {
				...
				mustAddDependency(t, database, issues[1].ID, issues[0].ID, "depends_on")
				mustAddDependency(t, database, issues[2].ID, issues[1].ID, "depends_on")
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 4; i++ {
				...
				mustCreateIssue(t, database, issues[i])
			}
	...
	tests := []struct {
	...
	}{
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				// A -> B, A -> C, B -> D, C -> D
				mustAddDependency(t, database, issues["B"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["C"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["D"].ID, issues["B"].ID, "depends_on")
				mustAddDependency(t, database, issues["D"].ID, issues["C"].ID, "depends_on")
			},
		...
		},
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				// A -> B -> C -> D -> E
				mustAddDependency(t, database, issues["B"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["C"].ID, issues["B"].ID, "depends_on")
				mustAddDependency(t, database, issues["D"].ID, issues["C"].ID, "depends_on")
				mustAddDependency(t, database, issues["E"].ID, issues["D"].ID, "depends_on")
			},
			checkFunc: func(t *testing.T, database *db.DB, issues map[string]*models.Issue) {
		...
		},
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				...
				mustAddDependency(t, database, issues["B"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["C"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["D"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["E"].ID, issues["A"].ID, "depends_on")
				mustAddDependency(t, database, issues["F"].ID, issues["A"].ID, "depends_on")
			},
			checkFunc: func(t *testing.T, database *db.DB, issues map[string]*models.Issue) {
