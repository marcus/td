...
func TestWouldCreateCycleSimple(t *testing.T) {
	...
	database.AddDependency(issue2.ID, issue1.ID, "depends_on") //nolint:errcheck // test setup
...
func TestWouldCreateCycleTransitive(t *testing.T) {
	...
	database.AddDependency(issue2.ID, issue1.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(issue3.ID, issue2.ID, "depends_on") //nolint:errcheck // test setup
...
func TestGetTransitiveBlockedChain(t *testing.T) {
	...
	database.CreateIssue(issue2) //nolint:errcheck // test setup
	database.CreateIssue(issue3) //nolint:errcheck // test setup
	database.CreateIssue(issue4) //nolint:errcheck // test setup

	database.AddDependency(issue2.ID, issue1.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(issue3.ID, issue2.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(issue4.ID, issue3.ID, "depends_on") //nolint:errcheck // test setup
...
func TestGetTransitiveBlockedDiamond(t *testing.T) {
	...
	database.CreateIssue(top) //nolint:errcheck // test setup
	database.CreateIssue(mid1) //nolint:errcheck // test setup
	database.CreateIssue(mid2) //nolint:errcheck // test setup
	database.CreateIssue(bottom) //nolint:errcheck // test setup

	database.AddDependency(mid1.ID, top.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(mid2.ID, top.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(bottom.ID, mid1.ID, "depends_on") //nolint:errcheck // test setup
	database.AddDependency(bottom.ID, mid2.ID, "depends_on") //nolint:errcheck // test setup
...
func TestBuildCriticalPathSkipsClosedIssues(t *testing.T) {
	...
	database.CreateIssue(openIssue) //nolint:errcheck // test setup
	database.CreateIssue(closedIssue) //nolint:errcheck // test setup
...
func TestDepAddDependsOnFlag(t *testing.T) {
	...
	// Reset
	depAddCmd.Flags().Set("depends-on", "") //nolint:errcheck // test setup
}
...
func TestAddDependencyMultiple(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			database.CreateIssue(mainIssue) //nolint:errcheck // test setup
	...
	tests := []struct {
	...
	}{
		{
			...
			setupChain: func(database *db.DB, issues []*models.Issue) {
				// issue1 -> issue2
				database.AddDependency(issues[1].ID, issues[0].ID, "depends_on") //nolint:errcheck // test setup
			},
		...
		},
		{
			...
			setupChain: func(database *db.DB, issues []*models.Issue) {
				...
				database.AddDependency(issues[1].ID, issues[0].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues[2].ID, issues[1].ID, "depends_on") //nolint:errcheck // test setup
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 4; i++ {
				...
				database.CreateIssue(issues[i]) //nolint:errcheck // test setup
			}
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(database *db.DB) (string, string) {
				...
				addDependency(database, issue1.ID, issue2.ID, "ses_test") //nolint:errcheck // test setup
...
func TestAddDependencyPersistence(t *testing.T) {
	...
	// Add dependencies
	addDependency(database, issue2.ID, issue1.ID, "ses_test") //nolint:errcheck // test setup
	addDependency(database, issue3.ID, issue2.ID, "ses_test") //nolint:errcheck // test setup
	...
	tests := []struct {
	...
	}{
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				// A -> B, A -> C, B -> D, C -> D
				database.AddDependency(issues["B"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["C"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["D"].ID, issues["B"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["D"].ID, issues["C"].ID, "depends_on") //nolint:errcheck // test setup
			},
		...
		},
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				// A -> B -> C -> D -> E
				database.AddDependency(issues["B"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["C"].ID, issues["B"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["D"].ID, issues["C"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["E"].ID, issues["D"].ID, "depends_on") //nolint:errcheck // test setup
			},
			checkFunc: func(t *testing.T, database *db.DB, issues map[string]*models.Issue) {
		...
		},
		{
			...
			buildGraph: func(database *db.DB, issues map[string]*models.Issue) {
				...
				database.AddDependency(issues["B"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["C"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["D"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["E"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
				database.AddDependency(issues["F"].ID, issues["A"].ID, "depends_on") //nolint:errcheck // test setup
			},
			checkFunc: func(t *testing.T, database *db.DB, issues map[string]*models.Issue) {
