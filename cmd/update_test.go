...
func TestUpdateReplaceDependencies(t *testing.T) {
	...
	mustCreateIssue(t, database, issue)
	mustCreateIssue(t, database, dep1)
	mustCreateIssue(t, database, dep2)

	// Add original dependency
	mustAddDependency(t, database, issue.ID, dep1.ID, "depends_on")
	...
	mustRemoveDependency(t, database, issue.ID, dep1.ID)
	mustAddDependency(t, database, issue.ID, dep2.ID, "depends_on")
...
func TestUpdateClearDependencies(t *testing.T) {
	...
	for _, dep := range deps {
		mustRemoveDependency(t, database, issue.ID, dep)
	}
...
func TestUpdateReplaceBlocks(t *testing.T) {
	...
	mustRemoveDependency(t, database, blocked1.ID, blocker.ID)
...
func TestUpdateCmdHasStatusFlag(t *testing.T) {
	...
	// Reset
	mustSetFlag(t, updateCmd.Flags(), "status", "")
}
