...
func TestCascadeCLIMultipleChildren(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		must(t, database.CreateIssue(children[i]))
	}
	...
	for i := 0; i < 2; i++ {
		...
		must(t, database.UpdateIssue(children[i]))
	}
	...
	must(t, database.UpdateIssue(children[2]))
...
func TestCascadeCLINestedHierarchy(t *testing.T) {
	...
	must(t, database.CreateIssue(grandparent))
...
func TestCascadeCLIInReviewStatus(t *testing.T) {
	...
	must(t, database.UpdateIssue(child1))
...
func TestCascadeCLIMixedStatusChildren(t *testing.T) {
	...
	now := time.Now()
	child3.ClosedAt = &now
	must(t, database.CreateIssue(child3))
