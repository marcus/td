...
func (v *Verifier) VerifyConvergence(actorA, actorB string) []VerifyResult {
	...
	strictChecks := []struct {
		...
		{"board_issue_positions", "SELECT bp.board_id, bp.issue_id, bp.position FROM board_issue_positions bp JOIN issues i ON bp.issue_id = i.id WHERE i.deleted_at IS NULL ORDER BY bp.board_id, bp.issue_id;"},
...
func (v *Verifier) dbContentHash(actor string) (string, error) {
	queries := []string{
		...
		"SELECT board_id, issue_id, position FROM board_issue_positions ORDER BY board_id, issue_id;",
