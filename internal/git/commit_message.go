...
var (
	commitSubjectPrefixPattern   = regexp.MustCompile(`^\s*([A-Za-z]+)\s*(?:\(\s*([^)]+?)\s*\))?\s*:\s*(.*)$`)
...
func NormalizeCommitType(raw string) (CommitType, error) {
	...
	switch normalized {
	case CommitTypeFeat, CommitTypeFix, CommitTypeDocs, CommitTypeTest, CommitTypeChore, CommitTypeCI:
