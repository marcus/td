...
func ExtractCommitIssueID(subject string) (string, error) {
	...
	issueID, _, err := stripTrailingIssueIDs(trimmed)
	if err != nil {
		return "", err
	}
	return issueID, nil
...
func NormalizeCommitSubject(subject string, opts CommitMessageOptions) (string, error) {
	parsed, err := parseCommitSubject(subject, opts.Type != "" || opts.IssueType != "")
	if err != nil {
		return "", err
	}
	...
	return formatCommitSubject(commitType, parsed.Scope, parsed.Breaking, parsed.Summary, issueID), nil
}
...
func parseCommitSubject(subject string, allowUnknownPrefix bool) (parsedCommitSubject, error) {
...
func formatCommitSubject(commitType CommitType, scope string, breaking bool, summary, issueID string) string {
