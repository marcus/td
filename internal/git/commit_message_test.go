...
func TestNormalizeCommitSubjectAllowsArbitraryWordPrefixWhenTypeOverrideIsSet(t *testing.T) {
	got, err := NormalizeCommitSubject("HTTP: retry on EOF", CommitMessageOptions{
		IssueID: "td-a1b2",
		Type:    CommitTypeFix,
	})
	if err != nil {
	...
	want := "fix: HTTP: retry on EOF (td-a1b2)"
	if got != want {
		t.Fatalf("NormalizeCommitSubject = %q, want %q", got, want)
	}
}

func TestNormalizeCommitSubjectAllowsArbitraryWordPrefixWhenIssueTypeProvidesDefault(t *testing.T) {
	got, err := NormalizeCommitSubject("HTTP: retry on EOF", CommitMessageOptions{
		IssueID:   "td-a1b2",
...
func TestExtractCommitIssueIDAllowsArbitraryWordPrefix(t *testing.T) {
	got, err := ExtractCommitIssueID("CLI: tighten help text (TD-A1B2)")
	if err != nil {
		t.Fatalf("ExtractCommitIssueID returned error: %v", err)
	}

	if got != "td-a1b2" {
		t.Fatalf("ExtractCommitIssueID = %q, want %q", got, "td-a1b2")
	}
}
