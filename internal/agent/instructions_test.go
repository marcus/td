...
func TestDetectAgentFile(t *testing.T) {
	t.Run("finds AGENTS.md first", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "AGENTS.md"), []byte("# Agents"), 0644) //nolint:errcheck // test setup
		os.WriteFile(filepath.Join(dir, "CLAUDE.md"), []byte("# Claude"), 0644) //nolint:errcheck // test setup
	...
	t.Run("finds GEMINI.md", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "GEMINI.md"), []byte("# Gemini"), 0644) //nolint:errcheck // test setup
	...
	t.Run("finds CLAUDE.local.md", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "CLAUDE.local.md"), []byte("# Local"), 0644) //nolint:errcheck // test setup
	...
	t.Run("finds CODEX.md", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "CODEX.md"), []byte("# Codex"), 0644) //nolint:errcheck // test setup
...
func TestHasTDInstructions(t *testing.T) {
	t.Run("returns true when file contains td usage", func(t *testing.T) {
		...
		os.WriteFile(path, []byte("Run td usage --new-session"), 0644) //nolint:errcheck // test setup
	...
	t.Run("returns false when file has no td usage", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "CLAUDE.md")
		os.WriteFile(path, []byte("# Claude instructions"), 0644) //nolint:errcheck // test setup
...
func TestAnyFileHasTDInstructions(t *testing.T) {
	t.Run("returns true when CLAUDE.md has instructions", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "CLAUDE.md"), []byte("Run td usage --new-session"), 0644) //nolint:errcheck // test setup
	...
	t.Run("returns true when GEMINI.md has instructions", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "GEMINI.md"), []byte("Use td usage -q"), 0644) //nolint:errcheck // test setup
	...
	t.Run("returns true when CLAUDE.local.md has instructions", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "CLAUDE.local.md"), []byte("td usage"), 0644) //nolint:errcheck // test setup
	...
	t.Run("returns true when CODEX.md has instructions", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "CODEX.md"), []byte("td usage --new-session"), 0644) //nolint:errcheck // test setup
	...
	t.Run("finds instructions in non-primary file", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "GEMINI.local.md"), []byte("td usage"), 0644) //nolint:errcheck // test setup
