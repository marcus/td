...
func TestBackfillStaleIssues_AddsUpdate(t *testing.T) {
	...
	var count int
	must(t, db.QueryRow(`SELECT COUNT(*) FROM action_log WHERE entity_id='td-700' AND action_type='create'`).Scan(&count))
	if count != 2 {
...
func TestBackfillOrphanEntities_SkipsAfterPull(t *testing.T) {
	...
	var count int
	must(t, db.QueryRow(`SELECT COUNT(*) FROM action_log`).Scan(&count))
	if count != 0 {
