...
func TestApplyRemoteEvents_PartialFailure(t *testing.T) {
	...
	var count int
	must(t, db.QueryRow("SELECT COUNT(*) FROM issues").Scan(&count))
	if count != 2 {
...
func TestMarkEventsSynced(t *testing.T) {
	...
	must(t, db.QueryRow("SELECT rowid FROM action_log WHERE id = ?", "al-00000001").Scan(&rowid1))
	must(t, db.QueryRow("SELECT rowid FROM action_log WHERE id = ?", "al-00000002").Scan(&rowid2))
	...
	must(t, db.QueryRow("SELECT synced_at, server_seq FROM action_log WHERE id = ?", "al-00000001").Scan(&syncedAt, &serverSeq))
	if !syncedAt.Valid {
	...
	must(t, db.QueryRow("SELECT synced_at, server_seq FROM action_log WHERE id = ?", "al-00000002").Scan(&syncedAt, &serverSeq))
	if !syncedAt.Valid {
	...
	// Verify unsynced row
	must(t, db.QueryRow("SELECT synced_at, server_seq FROM action_log WHERE id = ?", "al-00000003").Scan(&syncedAt, &serverSeq))
	if syncedAt.Valid {
