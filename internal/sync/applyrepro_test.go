...
func TestReproNullLabels_ExplicitNull(t *testing.T) {
  ...
  if !l.Valid { t.Errorf("labels NULL when key explicitly null -- THIS IS THE BUG") }
}
func TestReproNullLabels_EmptyArray(t *testing.T) {
  ...
  if !l.Valid { t.Errorf("labels NULL when array []") }
}
func TestReproNullLabels_EmptyString(t *testing.T) {
  ...
  if !l.Valid { t.Errorf("labels NULL when string ''") }
}
