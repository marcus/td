...
func (db *ServerDB) DeclineInvitation(invitationID, email string) error {
	...
	if err != nil {
		return fmt.Errorf("select invitation: %w", err)
	}
	if inv.Status != InvitationStatusPending {
	...
	if _, err := tx.Exec(`UPDATE invitations SET status = ? WHERE id = ?`, InvitationStatusDeclined, invitationID); err != nil {
...
// ForceExpireInvitationForTest sets expires_at to simulate expiry in tests.
func (db *ServerDB) ForceExpireInvitationForTest(id string, expiresAt time.Time) {
	_, _ = db.conn.Exec(`UPDATE invitations SET expires_at = ? WHERE id = ?`, expiresAt, id)
}
